...
def normalize_headers(raw_headers : str) -> dict : 
    mapping = {}
    unrecognized = []
...
def download_excel_to_memory(drive_service, file_meta: dict) -> io.BytesIO:
    ...
    Handles both a real uploaded .xlsx (get_media) and a native Google Sheet
    (must be exported instead — get_media doesn't work on those).
    """
